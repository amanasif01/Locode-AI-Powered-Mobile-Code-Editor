...
function normalize(input) {
  ...
  // Decode JSON-encoded source strings coming from the mobile bridge.
...
async function runWasmProgram(wasmBytes, stdinRaw = '', session = -1) {
  ...
  // reused by the next, so scanf/getchar don't trigger a new blocking XHR
  // for every character they read.
  ...
  const wasi = {
    ...
    fd_read(_fd, iovs, iovsLen, nwritten) {
      ...
      // This is essential: scanf/getchar/fgets all call fd_read multiple
      // times per line — once to fill libc's buffer, then again to peek at
      // delimiters.  Without this buffer every peek would block on a fresh XHR.
      ...
      try {
        ...
        if (xhr.status === 200 && xhr.responseText.length > 0) {
          // Store ALL fetched bytes; drain as many as fit the current iov now.
      ...
      // Return 0 bytes = EOF signal to libc (no input / XHR error).
      writeU32(nwritten, 0);
      return 0;
    },
...
// Inject setvbuf to disable stdout buffering so printf output appears
// before the program blocks on scanf/fgets/getchar — mirroring C++ behaviour.
function injectUnbufferedStdout(src) {
  // Find the opening brace of main() and insert setvbuf right after it.
  // Handles: int main(void), int main(), int main(int argc, char **argv), etc.
  return src.replace(
    /((?:int\s+main\s*\([^)]*\))\s*\{)/,
...
self.onmessage = async function (e) {
  ...
  const source = injectUnbufferedStdout(normalize(payload.code || ''));
  ...
  try {
    ...
    const files = await runClang(
      ['clang', '-O0', '-Wall', '-Wextra', 'main.c', '-o', 'main.wasm'],
      { 'main.c': source },
    ...
    if (outText.trim()) {
      self.postMessage(msg('stdout', outText.endsWith('\n') ? outText : `${outText}\n`, session));
    }
    if (errText.trim()) {
      self.postMessage(msg('stderr', errText.endsWith('\n') ? errText : `${errText}\n`, session));
    } else {
